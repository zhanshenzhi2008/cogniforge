...
> **注意（2026-08-20）**：忘记密码支持 Resend 邮件重置（未配置时回退管理员临时密码），见 API §9.1.2。  
> **注意（2026-08-19）**：会话列表/撤销、改密码必须走 `useApi`（带 JWT、打 Go），禁止 Nuxt `$fetch('/api/v1/...')`，否则会挂起转圈。细权限码、自定义角色**后期再排**，当前门禁仍是 `admin` / `user`。
...
**Q1：用户忘记密码怎么办？**
A：~~实现密码重置功能（通过邮箱验证码），暂不纳入阶段九范围。~~（2026-08-20）  
**当前**：
1. 已配置邮件（推荐 QQ SMTP：`SMTP_USER` + 授权码；或 Resend）→ 登录「忘记密码」发邮件链接，30 分钟内设新密码  
2. 未配置邮件 → 联系管理员；管理员在「用户」页「重置密码」生成临时密码  
**注意**：QQ SMTP 有日发送上限，只适合小流量；授权码不要提交到 Git。
