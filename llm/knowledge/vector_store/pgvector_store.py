...
class PGVectorStore(BaseVectorStore):
    ...
    def __init__(
        ...
        Args:
            host: PostgreSQL host (defaults to env vars)
    ...
    def create_collection(self, collection_name: str, dimension: int, **kwargs) -> None:
        ...
        """
        # Note: vector column will be added separately due to version differences
        
        # Create vector column (pgvector 0.5+)
        try:
            ...
            # Fallback: create table with vector in different format
            sql = f"""
            CREATE TABLE IF NOT EXISTS {table_name} (
                id VARCHAR(64) PRIMARY KEY,
                text_content TEXT NOT NULL,
        ...
        # Create HNSW index for faster search (pgvector 0.4+)
        try:
            ...
            # Fallback: create IVFFlat index
            try:
                self._cursor.execute(f"""
                    CREATE INDEX IF NOT EXISTS {table_name}_ivfflat_idx 
                    ON {table_name} 
                    USING ivfflat (embedding vector_cosine_ops)
                    WITH (lists = 100)
                """)
            except Exception as e2:
                logger.warning(f"Could not create IVFFlat index: {2}")
    ...
    def get_dimension(self, collection_name: str) -> Optional[int]:
        ...
        try:
            ...
            if result and result[0] > 0:
                return result[0] - 4  # Subtract 4 for pgvector header
        except Exception as e:
    ...
    def _sanitize_table_name(self, name: str) -> str:
        ...
        # Replace non-alphanumeric with underscores and prefix with kb_
        import re
