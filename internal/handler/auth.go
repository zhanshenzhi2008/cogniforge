...
func Register(c *gin.Context) {
	...
	response.Created(c, AuthData{Token: token, User: user})
}
...
func Login(c *gin.Context) {
	...
	if req.Email != "" {
		...
		response.BadRequest(c, "请输入邮箱或用户名")
		return
	}
	...
	response.Success(c, AuthData{Token: token, User: user})
}
...
func CreateApiKey(c *gin.Context) {
	...
	response.Created(c, gin.H{"id": apiKey.ID, "name": apiKey.Name, "key": apiKey.Key, "created_at": apiKey.CreatedAt})
}
...
func DeleteApiKey(c *gin.Context) {
	...
	if apiKey.UserID != userID {
		response.Forbidden(c, "无权删除此 API Key")
		return
	}

	if err := database.DB.Delete(&apiKey).Error; err != nil {
		response.InternalError(c, "删除 API Key 失败")
		return
	}
